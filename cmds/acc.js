...
module.exports = {
  ...
  async execute({ api, event, args }) {
    ...
    if (subCommand === "check") {
      if (!uid || isNaN(uid)) return send("❌ Invalid syntax. Use: `acc check <UID>`");
...
// Scheduled auto-check every hour
cron.schedule("0 * * * *", async () => {
  const api = global.api;
  const threadID = "YOUR_ADMIN_THREAD_ID"; // replace this with your real thread ID
  const messageID = null;

  if (api) {
    await checkFriendRequests(api, threadID, messageID);
  } else {
    console.error("Global API is not available for cron job.");
  }
});
